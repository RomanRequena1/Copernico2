...
import consumers.no_registral.objeto.infrastructure.consumer.ObjetoTributarioTransaction
...
class MessageTestkitUtilsMock(sujeto: ActorRef) {
...
//  )
  ...
  implicit class StartMessageProcessor(messageBroker: MessageProcessor with MessageProducer) {
    ...
    def startProcessing(topics: Set[ActorTransaction[_]] = Set.empty, sys: ActorSystem): Unit = {
      implicit val actorTransactionRequirements: ActorTransactionRequirements = ActorTransactionRequirements(
        executionContext = sys.getDispatcher,
        config = ConfigFactory.empty
      )
      ...
      (if (topics.isEmpty) {
        ...
        .foreach { transaction =>
          println("creando topico: " + transaction.topic)
...
object MessageTestkitUtilsMock {
