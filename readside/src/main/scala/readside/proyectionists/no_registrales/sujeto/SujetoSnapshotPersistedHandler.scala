...
class SujetoSnapshotPersistedHandler(
  ...
  override def processMessage(registro: SujetoSnapshotPersisted): Future[Response.SuccessProcessing] = {
    ...
    for {
        ...
        case Failure(exception) => log.error("Cumbia Dont persist sujeto " + exception )
        case Success(value) => log.error("Cumbia Persist sujeto " + value )
          connOracleReadsideToCass(registro.deliveryId.toString(),"sujeto")
      }
