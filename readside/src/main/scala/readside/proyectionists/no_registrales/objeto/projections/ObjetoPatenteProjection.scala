...
case class ObjetoPatenteProjection(
    event: ObjetoSnapshotPersisted
) extends UpdateReadSideProjection[ObjetoSnapshotPersisted] {

  def collectionName: String = "read_side.buc_objeto_patente"

  private val sojIdentificador2: String = event.registro
    .flatMap(_.SOJ_IDENTIFICADOR_2)
    .orElse(event.objetoId2)
    .getOrElse("")

  val keys: List[(String, Object)] = List(
    "soj_tipo_objeto" -> event.tipoObjeto,
    "soj_identificador_2" -> sojIdentificador2,
