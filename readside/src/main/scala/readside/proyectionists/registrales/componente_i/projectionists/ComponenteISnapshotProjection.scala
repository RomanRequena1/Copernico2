...
final case class ComponenteISnapshotProjection(
  ...
  val bobDetailsResult: Option[Map[String, String]] =
    decode[Map[String, String]](registro.get.BOB_OTROS_ATRIBUTOS.asJson.toString()).toOption
  ...
  val fromRegistro: Option[List[(String, Option[Serializable])]] = registro map { registro =>
    List(
      ...
      "bci_otros_atributos" -> Some(bobDetailsResult),
