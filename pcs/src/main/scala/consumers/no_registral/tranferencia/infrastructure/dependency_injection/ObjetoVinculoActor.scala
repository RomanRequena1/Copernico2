...
class ObjetoVinculoActor(requirements: MonitoringAndMessageProducer, objetoActorPropsOption: Option[Props] = None)
  ...
  override def setupHandlers(): Unit = {
    ...
    commandBus.subscribe[ObjetoVinculoCommands.CreateNewVinculoObjSujToTransf](new CreateNewVinculoObjSujToTransfHandler(this).handle)
    commandBus.subscribe[ObjetoVinculoCommands.CreateObjetoVinculoFromObj](new CreateVinculoObjSujToTransfHandler(this, requirements).handle)
    commandBus.subscribe[ObjetoVinculoCommands.UpdateVinculoObjSujToTransf](new UpdateVinculoObjSujToTransfHandler(this).handle)
