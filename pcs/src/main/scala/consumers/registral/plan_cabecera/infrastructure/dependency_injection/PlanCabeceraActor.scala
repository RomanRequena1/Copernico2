...
import consumers.registral.plan_cabecera.application.cqrs.commands.PlanCabeceraUpdateFromDtoHandler
...
import consumers.registral.plan_cabecera.domain.events.PlanCabeceraUpdatedFromDtoHandler
...
case class PlanCabeceraActor(state: PlanCabeceraState = PlanCabeceraState())(
  ...
  //commandBus.subscribe[PlanCabeceraCommands.PlanCabeceraRemoveFromDto](new PlanCabeceraRemoveHandler().handle)
